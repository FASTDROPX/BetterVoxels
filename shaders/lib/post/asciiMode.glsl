...
int[8] ppAsciiPattern(float luminance) {
    if (luminance > 0.9)      return int[8](0xF8, 0xF8, 0xF8, 0xF8, 0xF8, 0x00, 0x00, 0x00); // FULL
    else if (luminance > 0.8) return int[8](0x70, 0x90, 0x60, 0xB8, 0x88, 0x70, 0x00, 0x00); // @
    else if (luminance > 0.7) return int[8](0x20, 0x00, 0x38, 0x08, 0x70, 0x00, 0x00, 0x00); // ?
    else if (luminance > 0.6) return int[8](0xF0, 0x90, 0x90, 0x90, 0xF0, 0x00, 0x00, 0x00); // O
    else if (luminance > 0.5) return int[8](0x80, 0x80, 0xF0, 0x90, 0xF0, 0x00, 0x00, 0x00); // P
    else if (luminance > 0.4) return int[8](0x70, 0x50, 0x70, 0x00, 0x00, 0x00, 0x00, 0x00); // o
    else if (luminance > 0.3) return int[8](0x70, 0x40, 0x70, 0x00, 0x00, 0x00, 0x00, 0x00); // c
    else if (luminance > 0.2) return int[8](0x20, 0x20, 0x00, 0x20, 0x00, 0x00, 0x00, 0x00); // i
    else if (luminance > 0.1) return int[8](0x20, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00); // .
    else                      return int[8](0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00); // (empty)
}

#if PP_ASCII_EDGE_GLYPHS == 1
    // Directional glyphs for edges (- / | \), by Sobel gradient angle.
    int[8] ppAsciiPatternAngle(float angle) {
        float a = mod(angle, 6.2831853);
        if (a < 0.3927 || a > 5.8905)        return int[8](0x20, 0x20, 0x20, 0x20, 0x20, 0x00, 0x00, 0x00); // |
        else if (a < 1.1781)                 return int[8](0x80, 0x40, 0x20, 0x10, 0x08, 0x00, 0x00, 0x00); // /
        else if (a < 1.9635)                 return int[8](0x00, 0x00, 0xF8, 0x00, 0x00, 0x00, 0x00, 0x00); // -
        else if (a < 2.7489)                 return int[8](0x08, 0x10, 0x20, 0x40, 0x80, 0x00, 0x00, 0x00); // \
        else if (a < 3.5343)                 return int[8](0x20, 0x20, 0x20, 0x20, 0x20, 0x00, 0x00, 0x00); // |
        else if (a < 4.3197)                 return int[8](0x80, 0x40, 0x20, 0x10, 0x08, 0x00, 0x00, 0x00); // /
        else if (a < 5.1051)                 return int[8](0x00, 0x00, 0xF8, 0x00, 0x00, 0x00, 0x00, 0x00); // -
        else                                 return int[8](0x08, 0x10, 0x20, 0x40, 0x80, 0x00, 0x00, 0x00); // \
    }
#endif
...
vec3 ApplyAsciiMode(sampler2D src, vec2 uv) {
    ...
    int pattern[8];
        ...
        if (magnitude < 0.4) pattern = ppAsciiPattern(cellLuma);
        else pattern = ppAsciiPatternAngle(clamp(atan(gradY, gradX), 0.0, 6.2831853));
    #else
        pattern = ppAsciiPattern(cellLuma);
    #endif
